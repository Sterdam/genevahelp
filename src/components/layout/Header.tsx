    ...
    <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between z-30 relative">
      ...
      <div className="flex items-center gap-2">
        <div ref={langRef} className="relative">
          ...
          {langOpen && (
            <div className="absolute end-0 top-full mt-1 w-48 max-h-80 overflow-y-auto bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-50">
              {languages.map((lang) => (
                <button
                  key={lang.code}
                  onClick={() => changeLanguage(lang.code)}
                  className={`w-full text-start px-3 py-2 text-sm flex items-center gap-2 hover:bg-gray-50 transition-colors ${
